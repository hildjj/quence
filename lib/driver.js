...
export default class Driver {
  ...
  /**
   * @param {Point} _p The center of the text
   * @param {string} _str The text
   * @param {string} _klass Spaces-separated CSS classes
   * @param {number} [_angle] Angle in radians
   * @abstract
   */
  draw_label(_p, _str, _klass, _angle) {
    // Default is a no-op
  }
   ...
   * @param {string} _klasses
   * @virtual
   */
  draw_path(cmds, _klasses) {
    let d = null;
   ...
   * @param {number} _x
   * @param {number} _y
   * @param {number} _theta Angle in radians
   * @param {(t: T) => void} _func All of the things created in the func are
   *   transformed
   * @abstract
   */
  transform(_x, _y, _theta, _func) {
    // Pure virtual
