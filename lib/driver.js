...
Driver.prototype.draw_loop = function draw_loop(loop) {
  var pad = (loop.depth+1)*5 + (this.props.rung_width / 2);
  var lt = this.pos(0, loop.start).adjust(-pad, 0);
  this.group('Loop: ' + loop.msg, function() {
    this.draw_path([
      "M", lt.adjust(0, -15),
      "L", lt.adjust(35, -15),
      "L", lt.adjust(35, 0),
    ...
    this.draw_label(lt.adjust(5, -2), "loop", "start");

    if (loop.msg) {
      this.draw_label(lt.adjust(40, -2), loop.msg, "start");
    }
    ...
    this.draw_path([
      ...
      "L", lt.adjust(35, -15),
      "L", lt.adjust(35, 0),
      "L", this.pos(right, loop.start).adjust(pad, 0),
      "L", this.pos(right, loop.end).adjust(pad, 0),
      "L", this.pos(0, loop.end).adjust(-pad, 0),
      "Z"], "block");
  });
};
