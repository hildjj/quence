...
// Will be shimmed in web version
import fs from 'fs';
...
function expand_cache(props, str) {
  ...
  return str.replace(/\[([^[\]]+)]/g, (_m, p1) => props[p1]);
}
...
export default class SVGDriver extends Driver {
   ...
   * @param {(b: Builder) => void} func All of the things created in the func
   *   are transformed
   */
  transform(x, y, theta, func) {
    ...
    if (theta) {
      tform.push(`rotate(${Point.deg(theta)})`);
    }
    if (tform.length > 0) {
      this.group(null, g => {
