...
var Ladder = function() {
    ...
    var parse_endpoint = function(endpoint) {
        ...
        // is there a time?
        if (m[3]) {
            log.debug("Endpoint timepoint = " + m[3]);
            ...
            if (!isNaN(tmp)) {
                ...
                log.debug("Looking up timepoint " + m[3]);
                timepoint = m[3];
                // It must be a timepoint
                log.debug(JSON.stringify(timepoints));
                log.debug('"' + timepoint + '"');
                log.debug(timepoints[timepoint]);
                if (timepoints[m[3]] === undefined) {
                    log.debug.die("Undefined timepoint " + m[3]);
                }
                time = timepoints[m[3]];
            }
        }
    ...
    var compute_arrow = function(x, double_headed) {
        ...
        log.debug("Later time = " + later_time);
        current_time = later_time + (flags.advance || 1);

        log.debug("EVENT START = " + start.time + " END=" + end.time + " current_time=" + current_time);
    };
    ...
    var compute_ladder = function(desc) {
        ...
        desc_ = utils.deep_copy(desc);
        log.debug("Computing ladder");
        log.debug(JSON.stringify(desc));
        if (desc.title) {
            title = desc.title;
    ...
    if (desc.time_height)
        ...
        if (desc.participants) {
            participants = utils.deep_copy(desc.participants);
        }
