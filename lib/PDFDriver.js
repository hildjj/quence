...
PDFDriver.prototype.draw = function draw(outstream, cb) {
  var that = this;
  that.doc.pipe(outstream, {end: false});
...
PDFDriver.prototype.home_link = function home_link(pjson) {
  ...
  p = p.adjust(-wh[0]-3, 0);
  var extra = Object.assign({}, this.style("link"), {
    link: pjson.homepage,
    underline: "blue",
    wordCount: 1,
...
PDFDriver.prototype.draw_string = function draw_string(p, str, opts)
{
  ...
  if (opts.underline) {
    this.doc.underline(p.x, p.y, w, h, Object.assign({color: opts.underline}, opts));
  }
  if (opts.link) {
...
PDFDriver.prototype.draw_path = function draw_path(cmds, klasses) {
  ...
  this.doc.save();
  var extra = this.style("path " + klasses) || {};
  this.doc.path(d);
