...
module.exports = class PDFDriver extends Driver {
  constructor(...args) {
    super(...args);
    const p = this.props;
    this.styles = {
  ...
  draw(outstream, cb) {
    ...
    Driver.prototype.draw.call(this, (er) => {
      if (er != null) {
        cb(er);
      } else {
        that.doc.end();
  ...
  home_link(pjson) {
    ...
    p = p.adjust(-wh[0] - 3, 0);
    const extra = Object.assign({}, this.style('link'), {
      link: pjson.homepage,
      underline: 'blue',
      wordCount: 1,
  ...
  draw_string(p, str, opts) {
    ...
    if (opts.underline) {
      this.doc.underline(
        ...
        w,
        h,
        Object.assign({ color: opts.underline }, opts)
      );
    }
    if (opts.link) {
