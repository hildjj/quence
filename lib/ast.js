...
export class Message extends Step {
  ...
  constructor(line, tm, frm, arrow, to, msg, props) {
  ...
  compute(diag) {
    ...
    this.from.compute(diag);
    this.to.compute(diag, this.from.TM, this.props.duration);
    this.msg = diag.autoNumber(this.msg);

    diag.incrTime(this.from.TM, this.to.TM, this.props.advance);
  }
...
export class SelfMessage extends Message {
  ...
  // eslint-disable-next-line max-params
  constructor(line, tm, frm, arrow, to, msg, props) {
    super(line, tm, frm, arrow, to, msg, props);
    this.kind = Kind.SELF;
  }
  ...
  compute(diag) {
    ...
    this.from.compute(diag);
    const dur = Math.max(this.props.duration ?? 0, 1);
...
export class Diagram {
   ...
   */
  // eslint-disable-next-line max-params
  addMessage(line, tm, frm, arrow, to, msg, props) {
    let m = null;
    if (frm.nm === to.nm) {
      m = new SelfMessage(line, tm, frm, arrow, to, msg, props);
    } else {
      m = new Message(line, tm, frm, arrow, to, msg, props);
    }
